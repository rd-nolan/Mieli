...
/// 1. Workspace initialization, bookmark creation/persistence/restore, `.muisti/` isolation.
...
final class EndToEndWorkflowTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        let tempRoot = FileManager.default.temporaryDirectory
            .appendingPathComponent("muisti-e2e-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempRoot, withIntermediateDirectories: true)
        workspaceURL = tempRoot
        bookmarkFileURL = tempRoot.appendingPathComponent("muisti.bookmark")
    ...
    func testWorkspaceLifecycleAndBookmarkRestore() throws {
        // 1. Initialize .muisti directory
        let muistiDir = workspaceURL.appendingPathComponent(".muisti", isDirectory: true)
        try FileManager.default.createDirectory(at: muistiDir, withIntermediateDirectories: true)
        XCTAssertTrue(FileManager.default.fileExists(atPath: muistiDir.path))
        ...
        XCTAssertNotNil(workspaceManager.muistiDirectoryURL)
    }
    ...
    func testNoteCreationMetadataAndAtomicSave() throws {
        ...
        XCTAssertEqual(title, "Muisti 架构与状态机设计")
