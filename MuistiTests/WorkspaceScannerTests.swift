...
/// Markdown notes, ignores `.muisti` + `*.files`, and reports relative paths.
final class WorkspaceScannerTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        root = FileManager.default.temporaryDirectory
            .appendingPathComponent("muisti-scan-\(UUID().uuidString)", isDirectory: true)
    ...
    /// `.muisti`, legacy `.minne`, `*.files`, and non-markdown files.
    func testScanRecognizesStructureAndIgnoresInternal() throws {
        ...
        try makeDir(".muisti")                 // internal → skipped
        try makeFile(relative: ".muisti/index.sqlite")
        try makeDir(".minne")                  // legacy internal → skipped
