...
///
/// Recognizes real directories and `*.md` files. Ignores Minne-internal
/// directories (`.minne`) and attachment folders (`*.files`). Does not read or
...
enum WorkspaceScanner {
    ...
    private static func scanChildren(of dir: URL, relativePrefix: String) throws -> [WorkspaceItem] {
        ...
        // `skipsHiddenFiles` also skips `.minne`; keep an explicit guard anyway.
        return try contents
            .filter { !isIgnored($0) }
