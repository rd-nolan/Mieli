...
/// 1. Workspace initialization, bookmark creation/persistence/restore, `.minne/` isolation.
...
final class EndToEndWorkflowTests: XCTestCase {
    ...
    override func setUp() async throws {
        ...
        let tempRoot = FileManager.default.temporaryDirectory
            .appendingPathComponent("minne-e2e-\(UUID().uuidString)", isDirectory: true)
        try FileManager.default.createDirectory(at: tempRoot, withIntermediateDirectories: true)
        workspaceURL = tempRoot
        bookmarkFileURL = tempRoot.appendingPathComponent("minne.bookmark")
    ...
    func testWorkspaceLifecycleAndBookmarkRestore() throws {
        // 1. Initialize .minne directory
        let minneDir = workspaceURL.appendingPathComponent(".minne", isDirectory: true)
        try FileManager.default.createDirectory(at: minneDir, withIntermediateDirectories: true)
        XCTAssertTrue(FileManager.default.fileExists(atPath: minneDir.path))
        ...
        XCTAssertNotNil(workspaceManager.minneDirectoryURL)
    }
    ...
    func testNoteCreationMetadataAndAtomicSave() throws {
        ...
        XCTAssertEqual(title, "Minne 架构与状态机设计")
