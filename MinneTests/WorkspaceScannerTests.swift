...
/// Markdown notes, ignores `.minne` + `*.files`, and reports relative paths.
final class WorkspaceScannerTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        root = FileManager.default.temporaryDirectory
            .appendingPathComponent("minne-scan-\(UUID().uuidString)", isDirectory: true)
    ...
    /// `.minne`, `*.files`, and non-markdown files.
    func testScanRecognizesStructureAndIgnoresInternal() throws {
        ...
        try makeDir(".minne")                 // internal → skipped
