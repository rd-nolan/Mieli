...
/// T013 (`.muisti/` setup) is a separate task.
@Observable
final class WorkspaceManager {
    private static let internalDirectoryName = ".muisti"
    ...
    /// The internal `.muisti` directory within the workspace, or `nil`
    /// if no workspace is selected.
    var muistiDirectoryURL: URL? {
    ...
    /// `.muisti/index.sqlite` (created on first use) and indexes new / modified
...
case .modified:
    ...
    private func openIndex() {
        ...
        do {
            ...
            Logger(subsystem: "Muisti", category: "Index")
    ...
    /// directory (`.muisti`, legacy `.minne`, `*.files`) are rejected. On success the tree is
...
/// Flags that the currently-open note was changed by a source other than
    ...
    /// non-empty, not `.`/`..`, and not an internal name (`.muisti`, legacy
    /// `.minne`, `*.files`).
    ...
    /// directory (never `.muisti`/`*.files`, which `isValidWorkspacePath`
...
guard target.hasPrefix(rootResolved + "/") else { return nil }
    ...
    /// The folder must be a real directory under the workspace (`.muisti` and
    ...
    /// not `.`/`..`, and not an internal name (`.muisti`, `*.files`).
    ...
    /// segments, and nowhere under `.muisti`, legacy `.minne`, or a `*.files` folder.
    private func isValidWorkspacePath(_ path: String) -> Bool {
        ...
        // Reject internal segments, including the legacy `.minne` directory.
    ...
    /// Creates `<workspace>/.muisti` if it does not exist yet, migrating the
    /// legacy `.minne` directory when necessary.
    ///
    /// Idempotent: reuses an existing `.muisti` directory. If both names exist,
    /// the legacy directory is preserved rather than deleted. Returns `false`
    /// if no workspace is set or a migration/creation fails. Does not create
    /// SQLite (that is a later task).
    func ensureMuistiDirectory() -> Bool {
        guard let workspaceURL, let url = muistiDirectoryURL else { return false }
        let fileManager = FileManager.default
        let legacyURL = workspaceURL.appendingPathComponent(
        ...
        if !fileManager.fileExists(atPath: url.path),
           fileManager.fileExists(atPath: legacyURL.path) {
            do {
                ...
                Logger(subsystem: "Muisti", category: "Workspace")
    ...
    func selectWorkspace(prompt: String = String(localized: "Select Workspace")) -> Bool {
        ...
        workspaceURL = url
        guard ensureMuistiDirectory() else {
    ...
    func restoreWorkspace(bookmarkSource: URL? = nil) -> URL? {
        ...
        if let bookmarkSource {
            ...
            // Prefer the new location, but retain a fallback for users whose
            // workspace bookmark was stored before the app rename.
            sources = [Self.bookmarkURL, Self.legacyBookmarkURL]
        }
        ...
        for source in sources {
            ...
            if source == Self.legacyBookmarkURL {
    ...
    ///
/// Grants read-write access: Muisti needs to save, rename, delete and write
