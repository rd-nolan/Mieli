...
/// `.muisti` (or legacy `.minne`) and `*.files` per AGENTS §2/§20) and reports
/// Markdown files that
...
final class WorkspaceWatcher {
    ...
    /// Scans `root` for Markdown files (ignoring `.muisti`, legacy `.minne`, and
    /// `*.files`) returning
    ...
    private static func walk(_ dir: URL,
        ...
        for url in contents {
            ...
            if name == ".muisti" || name == ".minne" || name.hasSuffix(".files") { continue }
    ...
    static func isIgnoredPath(_ relativePath: String) -> Bool {
        for segment in relativePath.split(separator: "/") {
            ...
            if s == ".muisti" || s == ".minne" || s.hasSuffix(".files") { return true }
        }
        return false
    }
}
