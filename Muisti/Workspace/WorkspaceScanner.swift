...
///
/// Recognizes real directories and `*.md` files. Ignores Muisti-internal
/// directories (`.muisti` and legacy `.minne`) and attachment folders
...
enum WorkspaceScanner {
    ...
    /// Directories to skip entirely: `.muisti`, legacy `.minne`, and `*.files`.
    /// (attachment folders). Only directories match either rule.
    private static func isIgnored(_ url: URL) -> Bool {
        let name = url.lastPathComponent
        return name == ".muisti" || name == ".minne" || name.hasSuffix(".files")
    }
}
