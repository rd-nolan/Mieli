...
/// `.mieli` (or legacy `.muisti`/`.minne`) and `*.files` per AGENTS §2/§20) and reports
/// Markdown files that
...
final class WorkspaceWatcher {
    ...
    /// Scans `root` for Markdown files (ignoring `.mieli`, legacy `.muisti`/`.minne`, and
    /// `*.files`) returning
