...
final class WorkspaceDeleteFolderTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("mieli-delfld-\(UUID().uuidString)", isDirectory: true)
    ...
    func testRejectsInternalAndTraversalPaths() {
        XCTAssertFalse(manager.deleteFolder(at: ".mieli"))
        // The internal directory must be left untouched (never deleted).
        XCTAssertTrue(FileManager.default.fileExists(
            atPath: tempRoot.appendingPathComponent(".mieli").path))
