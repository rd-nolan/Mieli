...
final class WorkspaceCreateNoteTests: XCTestCase {
    ...
    override func setUpWithError() throws {
        tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("mieli-note-\(UUID().uuidString)", isDirectory: true)
    ...
    func testRenameRequestIsRejectedAfterWorkspaceChanges() {
        let original = URL(fileURLWithPath: "/tmp/mieli-original", isDirectory: true)
        let replacement = URL(fileURLWithPath: "/tmp/mieli-replacement", isDirectory: true)
    ...
    func testRenameRequestRemainsCurrentInOriginWorkspace() {
        let workspace = URL(fileURLWithPath: "/tmp/mieli-workspace", isDirectory: true)
    ...
    func testRejectsInternalDirectories() {
        XCTAssertFalse(manager.createNote(at: ".mieli/data"))
