...
struct AppLanguageTests {
    private func defaults() -> (UserDefaults, String) {
        let suite = "MieliTests.AppLanguage.\(UUID().uuidString)"
    ...
    @Test("legacy language preference migrates to Mieli")
    func legacyPreferenceMigrates() {
        let (defaults, suite) = defaults()
        defer { defaults.removePersistentDomain(forName: suite) }

        defaults.set("zh-Hans", forKey: "Muisti.AppLanguage")
    ...
    @Test("historical language preference migrates to Mieli")
    func historicalPreferenceMigrates() {
        let (defaults, suite) = defaults()
        defer { defaults.removePersistentDomain(forName: suite) }

        defaults.set("en", forKey: "Minne.AppLanguage")
        let language = AppLanguage(
            defaults: defaults,
            legacyDefaults: defaults,
            preferredLanguages: ["zh-Hans-CN"]
        ...
        #expect(language.selection == .english)
        #expect(defaults.string(forKey: AppLanguage.preferenceKey) == "en")
    }
