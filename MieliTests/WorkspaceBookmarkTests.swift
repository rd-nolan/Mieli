...
// MARK: - T013 .mieli directory

extension WorkspaceBookmarkTests {

    /// No workspace selected → ensure fails without crashing.
    @MainActor
    func testEnsureMieliDirectoryFailsWithoutWorkspace() {
        let manager = WorkspaceManager()
        XCTAssertFalse(manager.ensureMieliDirectory())
    }

    /// Restoring a workspace creates the `.mieli` directory.
    @MainActor
    func testRestoreCreatesMieliDirectory() throws {
        ...
        let mieliDir = tempDir.appendingPathComponent(".mieli", isDirectory: true)
        var isDir: ObjCBool = false
        XCTAssertTrue(FileManager.default.fileExists(atPath: mieliDir.path, isDirectory: &isDir))
        XCTAssertTrue(isDir.boolValue, ".mieli should be a directory")
    }

    /// Restoring a workspace migrates the legacy `.muisti` directory and keeps
    /// its contents intact.
    @MainActor
    func testRestoreMigratesLegacyMuistiDirectory() throws {
    ...
    /// Restoring a workspace also migrates the historical `.minne` directory.
    @MainActor
    func testRestoreMigratesHistoricalMinneDirectory() throws {
    ...
    /// An existing `.mieli` directory is reused, not recreated.
    @MainActor
    func testEnsureReusesExistingMieliDirectory() throws {
        let mieliDir = tempDir.appendingPathComponent(".mieli", isDirectory: true)
        ...
        let marker = mieliDir.appendingPathComponent("marker.txt")
        ...
        XCTAssertNotNil(manager.restoreWorkspace(bookmarkSource: source))
        XCTAssertTrue(manager.ensureMieliDirectory())
        XCTAssertTrue(FileManager.default.fileExists(atPath: marker.path),
                      "existing .mieli content must be preserved")
    }
}
