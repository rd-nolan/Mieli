...
const CARET_MARKER = "\u2503";
const RANGE_OPEN_MARKER = "\u2770";
const RANGE_CLOSE_MARKER = "\u2771";
function walkText(doc, options) {
  const { blockSeparator = "\n", leafText = "\u25A1" } = options != null ? options : {};
  const emissions = [];
  let firstBlock = true;
  const pushSeparator = (pos) => {
    if (firstBlock) {
      firstBlock = false;
      return;
    }
...
function posToTextOffset(doc, pos, options) {
  let offset = 0;
...
function textOffsetToPos(doc, offset, options) {
  let current = 0;
  let lastPos = 0;
  for (const emission of walkText(doc, options)) {
...
function getSelectionSnapshot(state, options) {
  const { doc, selection } = state;
  const text = walkText(doc, options).map((emission) => emission.text).join("");
  if (selection.empty) {
...
export { CARET_MARKER, RANGE_CLOSE_MARKER, RANGE_OPEN_MARKER, browser, calculateNodePosition, calculateTextPosition, cloneTr, customInputRules, customInputRulesKey, equalNodeType, findChildren, findChildrenByMark, findNodeInSelection, findParent, findParentNode, findParentNodeClosestToPos, findParentNodeType, findSelectedNodeOfType, flatten, getMarkFromSchema, getMarkRange, getNodeFromSchema, getSelectionSnapshot, isTextOnlySlice, markRule, nodeRule, posToDOMRect, posToTextOffset, textOffsetToPos };
